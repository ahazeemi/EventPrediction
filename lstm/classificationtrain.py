...
def split_events(gdelt_df, country_code, measure_name = 'NumArticles'):
    ...
    event_series = [gdelt_df[(gdelt_df.QuadClass==event_code) & (gdelt_df.Actor1Geo_CountryCode==country_code)][[measure_name]]
...
def main():
    ...
    if train:
        ...
        model.fit(X_train, y_train,
                  ...
                  , callbacks=[es])
        model.save('models/' +country_code + str(predict_for) + 'lstmclassification.h5')  # save the model
    ...
    if test:
        ...
        accuracy_score = metrics.accuracy_score(y_test, y_pred.round())
        print("* Classification report *\n{}".format( metrics.classification_report(y_test, y_pred.round())))
        print("Accuracy on test set: {:.2f}%".format(accuracy_score * 100.0))
